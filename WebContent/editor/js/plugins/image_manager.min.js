...
! function (a) {
    ...
    "use strict";
    if (a.extend(a.FroalaEditor.DEFAULTS, {
        ...
        }), a.FroalaEditor.PLUGINS.imageManager = function (b) {
            function c() {
            ...
            function f() {
                B.empty();
                for (var a = 0; K > a; a++) B.append('<div class="fr-list-column"></div>')
            }
            ...
            function i() {
                A.show(), B.find(".fr-list-column").empty(), b.opts.imageManagerLoadURL ? a.ajax({
            ...
            function k() {
                if (I < G.length && (B.outerHeight() <= C.outerHeight() + b.opts.imageManagerScrollOffset || C.scrollTop() + b.opts.imageManagerScrollOffset > B.outerHeight() - C.outerHeight())) {
                    H++;
                    for (var a = b.opts.imageManagerPageSize * (H - 1); a < Math.min(G.length, b.opts.imageManagerPageSize * H); a++) l(G[a])
                }
            }

            function l(c) {
                var d = new Image,
                    e = a('<div class="fr-image-container fr-empty fr-image-' + J++ + '" data-loading="' + b.language.translate("Loading") + '.." data-deleting="' + b.language.translate("Deleting") + '..">');
                p(!1), d.onload = function () {
                    ...
                    for (var i in c) "thumb" != i && "url" != i && "tag" != i && f.attr("data-" + i, c[i]);
            ...
            function q(c) {
                ...
                if (d(), b.image.showProgressBar(), U) U.trigger("click");
                else {
                    b.events.focus(!0), b.selection.restore();
                    var f = b.position.getBoundingRect(),
                        g = f.left + f.width / 2,
                        h = f.top + f.height;
                ...
                var i = {},
                    j = e.data();
                for (var k in j) "url" != k && "tag" != k && (i[k] = j[k]);
                b.undo.saveStep(), b.image.insert(e.data("url"), !1, i, U)
            }
            ...
            function s(c, d) {
                c >= 10 && 20 > c ? A.hide() : c >= 20 && 30 > c && a(".fr-image-deleting").removeClass("fr-image-deleting"), b.events.trigger("imageManager.error", [{
                    code: c,
                    message: T[c]
                }, d])
            }
            ...
            function t() {
                var a = E.find(".fr-modal-title-line").outerHeight(),
                    b = D.outerHeight();
            ...
            var z, A, B, C, D, E, F, G, H, I, J, K, L = 10,
                M = 11,
                N = 12,
                O = 13,
                P = 14,
        ...
        }, !a.FroalaEditor.PLUGINS.image) throw new Error("Image manager plugin requires image plugin.");
    a.FroalaEditor.DEFAULTS.imageInsertButtons.push("imageManager"), a.FroalaEditor.RegisterCommand("imageManager", {
        title: "Browse",
        undo: !1,
        focus: !1,
        callback: function () {
        ...
        plugin: "imageManager"
    }), a.FroalaEditor.DefineIcon("imageManager", {
        NAME: "folder"
    })
});